...
bool CCScale9SpriteEx::initWithFile( const char* file,ccScale9Mode mode/*=eScale9Strach*/, const CCRect* rect/*= NULL*/, const CCRect* capInsets /*= NULL*/ )
{
	CCAssert(file != NULL, "Invalid file for sprite");

	scaleMode_ = mode;
	if(scaleMode_==eScale9Strach)
		groupNode_ = CCSpriteBatchNode::batchNodeWithFile(file, 9);
	else
		groupNode_ = CCNode::node();
	...
	if (!rect || CCRect::CCRectEqualToRect(*rect, CCRectZero)){
	...
	if (!capInsets || CCRect::CCRectEqualToRect(*capInsets, CCRectZero)) {
	...
	// center
	if(scaleMode_==eScale9Strach){
		spriteCentre_ = CCSprite::spriteWithTexture(texture, capInsets_);
		spriteTop_    = CCSprite::spriteWithTexture(texture, CCRectMake(capInsets_.origin.x, t, capInsets_.size.width, capInsets_.origin.y-t));
		spriteBottom_ = CCSprite::spriteWithTexture(texture, CCRectMake(capInsets_.origin.x, capInsets_.origin.y+capInsets_.size.height, capInsets_.size.width, h-(capInsets_.origin.y-t+capInsets_.size.height)));
		spriteLeft_   = CCSprite::spriteWithTexture(texture, CCRectMake(l, capInsets_.origin.y, capInsets_.origin.x-l, capInsets_.size.height));
		spriteRight_  = CCSprite::spriteWithTexture(texture, CCRectMake(capInsets_.origin.x + capInsets_.size.width,capInsets_.origin.y,w-(capInsets_.origin.x-l+capInsets_.size.width),capInsets_.size.height));
	} else if(scaleMode_ ==eScale9Tile){
		CCImage image;
		initImage(&image,file);
		spriteCentre_ = CCSprite::spriteWithTexture(CCTextureCache::sharedTextureCache()->addTextureBlock(&image,capInsets_.origin, capInsets_.size,file,Texture2DBlock_Center));
		spriteTop_    = CCSprite::spriteWithTexture(CCTextureCache::sharedTextureCache()->addTextureBlock(&image,CCPointMake(capInsets_.origin.x,t),CCSizeMake(capInsets_.size.width,capInsets_.origin.y - t),file,Texture2DBlock_Top));
		spriteBottom_ = CCSprite::spriteWithTexture(CCTextureCache::sharedTextureCache()->addTextureBlock(&image,CCPointMake(capInsets_.origin.x,capInsets_.origin.y + capInsets_.size.height),CCSizeMake(capInsets_.size.width,h-(capInsets_.origin.y-t+capInsets_.size.height)),file,Texture2DBlock_Bottom));
		spriteLeft_   = CCSprite::spriteWithTexture(CCTextureCache::sharedTextureCache()->addTextureBlock(&image,CCPointMake(l,capInsets_.origin.y),CCSizeMake(capInsets_.origin.x-l,capInsets_.size.height),file,Texture2DBlock_Left));
		spriteRight_  = CCSprite::spriteWithTexture(CCTextureCache::sharedTextureCache()->addTextureBlock(&image,CCPointMake(capInsets_.origin.x+capInsets_.size.width,capInsets_.origin.y),CCSizeMake(w-(capInsets_.origin.x - l + capInsets_.size.width),capInsets_.size.height),file,Texture2DBlock_Right));
	} else{
		CCAssert(false,"Invalid Mode.");
		return false;
	}

	spriteTopLeft_     = CCSprite::spriteWithTexture(texture, CCRectMake(l,t,capInsets_.origin.x-l,capInsets_.origin.y-t));
	spriteTopRight_    = CCSprite::spriteWithTexture(texture, CCRectMake(capInsets_.origin.x+capInsets_.size.width,t,w-(capInsets_.origin.x-l+capInsets_.size.width), capInsets_.origin.y-t));
	spriteBottomLeft_  = CCSprite::spriteWithTexture(texture, CCRectMake(l,capInsets_.origin.y+capInsets_.size.height, capInsets_.origin.x-l, h-(capInsets_.origin.y-t+capInsets_.size.height)) );
	spriteBottomRight_ = CCSprite::spriteWithTexture(texture, CCRectMake(capInsets_.origin.x+capInsets_.size.width, capInsets_.origin.y + capInsets_.size.height, w-(capInsets_.origin.x-l+capInsets_.size.width),h-(capInsets_.origin.y-t+capInsets_.size.height)));
...
CCScale9SpriteEx* CCScale9SpriteEx::spriteWithFile( const char* file,ccScale9Mode mode/*=eScale9Strach*/, const CCRect* rect/*=NULL*/, const CCRect* capInsets /*= NULL*/ )
{
...
void CCScale9SpriteEx::setIsOpacityModifyRGB( bool bValue )
{
	...
	if (groupNode_ && children && children->count()) {
		...
		CCARRAY_FOREACH(children, child){
			...
			if (pNode){
				pNode->setIsOpacityModifyRGB(bIsOpacityModifyRGB_);
			}
		}
	}
}
